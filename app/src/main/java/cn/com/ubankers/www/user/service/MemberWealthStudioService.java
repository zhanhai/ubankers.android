...
public class MemberWealthStudioService {
	...
	private void getUserFace(AttributesBean attributesBean) {
		...
		if (attributesBean != null && attributesBean.getName() != null&& attributesBean.getName().equals("fileid")&&attributesBean.getCatalog().equals("userface")) {
			MyApplication.loadImage(HttpConfig.HTTP_IMAGE_QUERY_URL+ attributesBean.getValue(), ((MemberWealthStudioActivity) context).faceImage,null);
		}else {
			((MemberWealthStudioActivity) context).faceImage.setBackgroundResource(R.drawable.personal_center);
		}
	}
