...
//		}else if(Tools.rangeInDefined(Integer.parseInt(money),Integer.parseInt(product.getMinMoney()+"0000"),Integer.parseInt(product.getMaxMoney()+"0000"))==false|| !(Integer.parseInt(money)%Integer.parseInt(increment_amount.getText().toString().trim())==0)){
			...
			pds.cfmpOrder(clientId, isBInvestor, money, number, name, dialog);
	    }
	}
	...
	private class MyOnClickListenerNickName implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			intent.putExtra(ProductDetailActivity.EXTRA_CLIENT_TYPE, type);
			((ProductDetailActivity) context).startActivityForResult(intent, ProductDetailActivity.REQUEST_CFMP_SELECT_INVESTOR);
		}
		
	}

	public void onInvestorSelected(int resultCode, Intent data){
		if( resultCode != Activity.RESULT_OK) {
			return;
		}

		int investorType = data.getIntExtra(ProductDetailActivity.EXTRA_CLIENT_TYPE, 0);

        if (investorType == ProductDetailActivity.INVESTOR_TYPE_REGISTERED) {

            registered_name.setText(data.getStringExtra(ProductDetailActivity.EXTRA_CLIENT_NAME));
            clientId = data.getStringExtra(ProductDetailActivity.EXTRA_CLIENT_ID);
            etNumber.setText(clientId);
            registered_name.clearFocus();
            registered_name.setFocusable(false);
            MyApplication.app.setOrderInvestorId(data.getStringExtra(ProductDetailActivity.EXTRA_CLIENT_MOBILE));

        }else if(investorType == ProductDetailActivity.INVESTOR_TYPE_B){
            registered_name.setText(data.getStringExtra(ProductDetailActivity.EXTRA_CLIENT_NAME));
            etNumber.setText(data.getStringExtra(ProductDetailActivity.EXTRA_CLIENT_MOBILE));
            registered_name.clearFocus();
            registered_name.setFocusable(false);
        }
